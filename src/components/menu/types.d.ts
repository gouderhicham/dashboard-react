...
import { CSSProperties, HTMLAttributes, MouseEvent, ReactNode, RefAttributes, RefObject } from 'react';
...
export interface IMenuOffsetModifier {
  name: 'offset';
  options?: { offset?: [number, number] };
}

export type TMenuDropdownModifier = IMenuOffsetModifier | { name: string; options?: unknown };

export interface TMenuDropdown {
  placement?: TMenuPlacement;
  modifiers?: TMenuDropdownModifier[];
  className?: string;
  style?: CSSProperties;
}
