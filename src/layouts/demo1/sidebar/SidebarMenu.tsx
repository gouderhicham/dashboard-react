    ...
    return (
      <MenuItem key={index}>
        ...
        >
          ...
          <MenuTitle className="text-sm font-medium text-gray-800">{t(item.title)}</MenuTitle>
          ...
          >
            ...
            {item.collapse ? (
              <MenuTitle className="text-2sm font-normal text-gray-600 dark:text-gray-500">
                <span className="hidden menu-item-show:!flex">{t(item.collapseTitle)}</span>
                <span className="flex menu-item-show:hidden">{t(item.expandTitle)}</span>
              </MenuTitle>
            ) : (
    ...
    return (
      <MenuItem key={index}>
        ...
        >
          ...
          <MenuTitle className="text-2sm font-normal text-gray-800">{t(item.title)}</MenuTitle>
          {item.disabled && buildMenuSoon()}
        </MenuLabel>
      </MenuItem>
    );
  ...
  const buildMenuArrow = () => {
    return (
      <MenuArrow className={clsx('text-gray-400 w-[20px] shrink-0 justify-end ms-1', rightOffset)}>
        <Plus className="size-3.5 menu-item-show:hidden" />
        <Minus className="size-3.5 hidden menu-item-show:inline-flex" />
      </MenuArrow>
    );
  };
